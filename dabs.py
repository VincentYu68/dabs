...
def read_state(port_num):
    ...
    for index, (id, attr) in enumerate(read_list):
        ...
        if attr[1] == 1:
            val = dynamixel.read1ByteTxRx(port_num, PROTOCOL_VERSION, id, attr[0])
        elif attr[1] == 2:
            val = dynamixel.read2ByteTxRx(port_num, PROTOCOL_VERSION, id, attr[0])
        elif attr[2] == 4:
            val = dynamixel.read4ByteTxRx(port_num, PROTOCOL_VERSION, id, attr[0])
        else:
...
def write_positions(port_num, targets):
    ...
    for index, (id, attr) in enumerate(write_list):
        ...
        if attr[1] == 1:
            dynamixel.write1ByteTxRx(port_num, PROTOCOL_VERSION, id, attr[0], targets[index])
        elif attr[1] == 2:
            dynamixel.write2ByteTxRx(port_num, PROTOCOL_VERSION, id, attr[0], targets[index])
        elif attr[2] == 4:
            dynamixel.write4ByteTxRx(port_num, PROTOCOL_VERSION, id, attr[0], targets[index])
        else:
            raise RuntimeError("Invalid data size")
        check_comm_error(port_num)
...
if __name__ == "__main__":
    ...
    print(read_state(port_num))
    write_positions(port_num, [0, 700])
